...
namespace V2HHTMiddleware.Controllers.HHT
{
    ...
    public class HHTController : ApiController
    {
        ...
        // Returns {code: name} JSON for all ~499 stores/hubs/DCs.
        // Android caches this in memory for the session.
        // Supabase is only called if cache is empty or >24h old.
        // ══════════════════════════════════════════════════════════════════════
        [HttpGet, Route("plantnames")]
        public async Task<IHttpActionResult> GetPlantNames()
        {
            var map = await EnsurePlantCacheAsync();
            return Ok(map);
        }

        // POST api/hht/refresh-plants  (call to force a fresh Supabase fetch)
        [HttpPost, Route("refresh-plants")]
        public async Task<IHttpActionResult> RefreshPlants()
        {
            _plantCache = null;
            var map = await EnsurePlantCacheAsync();
            return Ok(new { refreshed = true, count = map.Count, loadedAt = _plantLoadedAt });
        }

        // ── Internal: load from Supabase if cache is empty or stale ────────────
        private static async Task<Dictionary<string, string>> EnsurePlantCacheAsync()
        {
            // Return cached data if fresh (< 24 hours old)
            if (_plantCache != null && (DateTime.UtcNow - _plantLoadedAt).TotalHours < 24)
                return _plantCache;
            ...
            try
            {
                ...
                using (var req = new HttpRequestMessage(HttpMethod.Get, SUPABASE_PLANT_URL))
                {
                    ...
                    if (!resp.IsSuccessStatusCode)
                    {
                        ...
                        return _plantCache ?? new Dictionary<string, string>();
                    }
                    ...
                    {
                        string code = row.ContainsKey("STORE-CODE") ? (row["STORE-CODE"] ?? "").Trim().ToUpper() : "";
                        string name = row.ContainsKey("STORE-NAME") ? (row["STORE-NAME"] ?? "").Trim() : "";
                        if (!string.IsNullOrEmpty(code) && !string.IsNullOrEmpty(name))
                            map[code] = name;
                    }

                    _plantCache    = map;
                    _plantLoadedAt = DateTime.UtcNow;
                    return map;
                }
            }
            catch (Exception ex)
            {
                System.Diagnostics.Debug.WriteLine("PlantNames fetch error: " + ex.Message);
                return _plantCache ?? new Dictionary<string, string>();
            }
            finally
            {
